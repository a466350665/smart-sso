...
<jsp:include page="../common/common.jsp">
	<jsp:param name="title" value="${empty office.id ? '添加' : '修改'}机构"/>
</jsp:include>

<div class="page-header">
	<h1>
		${empty office.id ? '添加' : '修改'}机构
	</h1>
</div>
				...
				<div class="col-sm-3">
					<select id="_parentId" name="parentId" class="form-control help-validate">
						...
						<c:forEach var="item" items="${officeList}">
							<option value="${item.id}" ${(item.id eq office.parentId)?'selected="selected"':''}>${item.name}</option>
						</c:forEach>
					</select>
				</div>
			</div>
					...
					<div class="clearfix help-validate">
						<div>
							<label class="line-height-1 blue">
								<input name="isEnable" value="true" type="radio" class="ace" ${office.isEnable ? 'checked="checked"' : ''}/>
						...
						<div>
							<label class="line-height-1 blue">
								<input name="isEnable" value="false" type="radio" class="ace" ${!office.isEnable ? 'checked="checked"' : ''}/>
		...
		jQuery(function($) {
			...
			$("#_submit").click(function(){
				if($('#_editForm').validate()){
					var btn = $(this);
					btn.button('loading');
					$.post("${_path}/admin/office/save", $.formJson('_editForm'),function(d) {
						if(d){
							btn.button('reset');
							if(d.code == 1){
								$.aceRedirect("${_path}/admin/office");
							}
							else {
