	...
	var a = {
			...
			return this
					.each(function() {
							...
							j
									.bind(
											...
											function() {
												...
												d.queue.selected = l;
												if ((d.queue.count + l) > f.queueSizeLimit
														&& f.queueSizeLimit !== 0) {
													if (b.inArray("onError",
															f.overrideEvents) < 0) {
														alert("The maximum number of queue items has been reached ("
																+ f.queueSizeLimit
						...
						d.drop = function(m) {
							...
							d.queue.selected = j;
							if ((d.queue.count + j) > f.queueSizeLimit
									&& f.queueSizeLimit !== 0) {
						...
						d.addQueueItem = function(j) {
							if (b.inArray("onAddQueueItem", f.overrideEvents) < 0) {
								...
								j.queueItem.attr("id", f.id + "-file-"
										+ d.fileID++);
								j.queueItem.find(".close").bind("click",
										function() {
											a.cancel.call(g, j);
											return false;
										});
								var k = j.name;
								if (k.length > f.truncateLength
										&& f.truncateLength != 0) {
						...
						d.progress = function(l, j) {
							if (b.inArray("onProgress", f.overrideEvents) < 0) {
								if (l.lengthComputable) {
									var k = Math
											.round((l.loaded / l.total) * 100);
								}
								j.queueItem.find(".fileinfo").html(
										" - " + k + "%");
								j.queueItem.find(".progress-bar").css("width",
										k + "%");
							}
						...
						d.error = function(l, j, k) {
							if (b.inArray("onError", f.overrideEvents) < 0) {
								...
								j.queueItem.addClass("error").find(".fileinfo")
										.html(" - " + errorMsg);
						...
						if (window.File && window.FileList && window.Blob
							...
							if (!f.queueID) {
								...
								d.queueEl = b('<div id="' + f.queueID
										+ '" class="uploadifive-queue" />');
								d.button.after(d.queueEl);
							} else {
								d.queueEl = b("#" + f.queueID);
							}
							if (f.dnd) {
								var h = f.dropTarget ? b(f.dropTarget)
										: d.queueEl.get(0);
		...
		cancel : function(d, c) {
			this.each(function() {
				...
				if (typeof d === "string") {
					if (!isNaN(d)) {
						fileID = "uploadifive-" + b(this).attr("id") + "-file-"
								+ d;
					}
			...
			this
					.each(function() {
						...
						if (c) {
							...
							if ((e.uploads.count + e.uploads.current) < f.uploadLimit
									|| f.uploadLimit == 0) {
								if (!d) {
								...
								b("#" + f.queueID)
										...
										.each(
												function() {
													_file = b(this)
															.data("file");
													if ((e.uploads.current >= f.simUploadLimit && f.simUploadLimit !== 0)
															|| (e.uploads.current >= f.uploadLimit && f.uploadLimit !== 0)
															|| (e.uploads.count >= f.uploadLimit && f.uploadLimit !== 0)) {
														return false;
													}
													if (f.checkScript) {
														_file.checking = true;
